        ...
        <?php if (($items_in_menu && $item['in_menu']) || (!$items_in_menu && !$item['in_menu'])): ?>
            <li <?php echo $item['in_menu'] ? 'class="item"' : 'class="item-menu"'; ?>>
                <a href="<?php echo url_for($item['url']) ?>" title="<?php echo $item['name']; ?>">
                    ...
                    <span>
                        <?php echo $item['name']; ?>
                        <?php if (AdminItemPeer::hasMessageByUrl($item['url'])) { ?>
                            <div class="admin_item_message_show">
                                <a href="<?php echo url_for($item['url'] . '/index/unread/1') ?>"><?php echo AdminItemPeer::getMessageNumByUrl($item['url']); ?></a>
                            </div>
                        <?php } ?>
                    </span>
                </a>
