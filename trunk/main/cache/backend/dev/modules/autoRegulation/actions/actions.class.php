...
abstract class autoRegulationActions extends sfActions
{
  ...
  public function executeBatch(sfWebRequest $request)
  {
    ...
    $validator = new sfValidatorPropelChoice(array('multiple' => true, 'model' => 'Regulation'));
    try
    {
      // validate ids
  ...
  protected function executeBatchDelete(sfWebRequest $request)
  {
    ...
    $count = 0;
    foreach (RegulationPeer::retrieveByPks($ids) as $object)
    {
  ...
  protected function processForm(sfWebRequest $request, sfForm $form)
  {
    ...
    if ($form->isValid())
    {
      ...
      if ($request->hasParameter('_save_and_add'))
      {
        ...
        $this->redirect('@regulation_new');
      }
      else
      {
        $this->getUser()->setFlash('notice', $notice);

        $this->redirect(array('sf_route' => 'regulation_edit', 'sf_subject' => $Regulation));
      }
    }
    else
    {
  ...
  protected function processSort($query)
  {
    ...
    try
    {
      $column = RegulationPeer::translateFieldName($sort[0], BasePeer::TYPE_FIELDNAME, BasePeer::TYPE_PHPNAME);
    }
  ...
  {
    $sort = $this->getUser()->getAttribute('regulation.sort', null, 'admin_module');
    if (null === $sort)
    {
