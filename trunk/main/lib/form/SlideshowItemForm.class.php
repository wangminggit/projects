...
class SlideshowItemForm extends BaseSlideshowItemForm {
    ...
    private function getImageLinkHtml() {
        if ($this->getObject()->getImageToken()) {
            ...
            if (file_exists($basepath . DIRECTORY_SEPARATOR . $this->getObject()->getImageToken())) {
                $width = ($this->getObject()->getSlideshow()->getImageWidth() > 500) ? ceil($this->getObject()->getSlideshow()->getImageWidth() / 3) : $this->getObject()->getSlideshow()->getImageWidth();
                $result = '<img src="/uploads/slideshow_item/' . $this->getObject()->getId() . '/' . $this->getObject()->getImageToken() . '" width="' . $width . ' "/><br /><br />';
            } else {
    ...
    private function processImage() {
        ...
        // upload if need
        if(isset($_FILES) && isset($_FILES['slideshow_item'])){
