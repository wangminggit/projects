...
class AboutusForm extends BaseAboutusForm {
    ...
    public function configure() {
        ...
        $this->validatorSchema['aboutus_category_id'] = new sfValidatorString(array('required' => true));
    ...
    private function getImageLinkHtml() {
        if ($this->getObject()->getImage()) {
            ...
            if (file_exists($basepath . $this->getObject()->getImage())) {
                $result = '<img src="/uploads/aboutus/' . $this->getObject()->getId() . '/image/' . $this->getObject()->getImage() . '" /><br /><br />';
            } else {
    ...
    public function doSave($con = null) {
        ...
        if ((isset($_POST['aboutus']['image_delete']) || (isset($_FILES['aboutus']['name']['image']) && $_FILES['aboutus']['name']['image'] != '')) && !$o->isNew()) {
            $o->deleteImage();
        }
    ...
    private function processImage() {
        $files = $_FILES['aboutus'];
        if (isset($_FILES['aboutus']) && $files['error']['image'] == UPLOAD_ERR_OK) {
            // 保存图片
            $o = $this->getObject();
