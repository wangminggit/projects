...
class InformationForm extends BaseInformationForm {
    ...
    public function configure() {
        ...
        $this->validatorSchema['information_category_id'] = new sfValidatorString(array('required' => true));
    ...
    private function getImageLinkHtml() {
        if ($this->getObject()->getImage()) {
            ...
            if (file_exists($basepath . $this->getObject()->getImage())) {
                $result = '<img src="/uploads/information/' . $this->getObject()->getId() . '/image/' . $this->getObject()->getImage() . '" /><br /><br />';
            } else {
    ...
    public function doSave($con = null) {
        ...
        if ((isset($_POST['information']['image_delete']) || (isset($_FILES['information']['name']['image']) && $_FILES['information']['name']['image'] != '')) && !$o->isNew()) {
            $o->deleteImage();
        }
    ...
    private function processImage() {
        $files = $_FILES['information'];
        if (isset($_FILES['information']) && $files['error']['image'] == UPLOAD_ERR_OK) {
            // 保存图片
            $o = $this->getObject();
