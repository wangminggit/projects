...
class mysfValidatorFileImage extends sfValidatorFile {
    ...
    protected function configure($options = array(), $messages = array()) {
        ...
        $this->addOption('square', false);
        /* we set a default value as image for mime type */
        $this->addOption('mime_types', 'web_images');


        /* size image error messages */
        $this->addMessage('min_size_x', '图片太小了 (最小宽度 是 %min_size_x% 像素).');
        $this->addMessage('max_size_x', '图片太宽了 (最大宽度 是 %max_size_x% 像素).');
        $this->addMessage('min_size_y', '图片太小了 (最小高度 是 %min_size_y% 像素).');
        $this->addMessage('max_size_y', '图片太高了 (最大高度 是 %max_size_y% 像素).');
        $this->addMessage('square', '图片不是个正方形.');
...
// END configure()
    ...
    protected function doClean($value) {
        ...
        // check square
        if ((int) $size[0] != (int) $size[1] && !$this->hasOption('size_x') && !$this->hasOption('size_y')) {
        }
        
        if ($this->hasOption('square') && $this->getOption('square') == true && (int) $size[0] != (int) $size[1] && !$this->hasOption('size_x') && !$this->hasOption('size_y')) {
            throw new sfValidatorError($this, 'square');
        }
