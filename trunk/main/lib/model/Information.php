...
/**
 * Skeleton subclass for representing a row from the 'information' table.
 *
 * 
 *
...
class Information extends BaseInformation {
    ...
    public function getImagePath() {
        $filename = "/uploads/information/" . $this->getId() . "/image/" . $this->getImage();
        return $filename;
    }

    public function getImageBasePath() {
        return sfConfig::get('sf_upload_dir') . DIRECTORY_SEPARATOR . 'information' . DIRECTORY_SEPARATOR . $this->getId() . DIRECTORY_SEPARATOR . 'image' . DIRECTORY_SEPARATOR;
    }
    ...
    public function getImageRelativePath() {
        ...
        if ($this->getImage()) {
            ...
            if (file_exists($basepath . $this->getImage())) {
                $result = '/uploads/information/' . $this->getId() . '/image/' . $this->getImage();
            } else {
    ...
    private function hookBeforeDelete() {
        $path = sfConfig::get('sf_upload_dir') . DIRECTORY_SEPARATOR . 'information' . DIRECTORY_SEPARATOR . $this->getId() . DIRECTORY_SEPARATOR;
    ...
    public function getNextItem() {
        $c = new Criteria();
        $c->add(InformationPeer::ID, $this->getId(), Criteria::LESS_THAN);
        $c->addDescendingOrderByColumn(InformationPeer::ID);
        $c->setLimit(1);
        return InformationPeer::doSelectOne($c);
    }
