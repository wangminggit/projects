...
/**
 * Skeleton subclass for representing a row from the 'aboutus' table.
 *
 * 
 *
...
class Aboutus extends BaseAboutus {
    ...
    public function getImagePath() {
        $filename = "/uploads/aboutus/" . $this->getId() . "/image/" . $this->getImage();
        return $filename;
    }

    public function getImageBasePath() {
        return sfConfig::get('sf_upload_dir') . DIRECTORY_SEPARATOR . 'aboutus' . DIRECTORY_SEPARATOR . $this->getId() . DIRECTORY_SEPARATOR . 'image' . DIRECTORY_SEPARATOR;
    }
    ...
    public function getImageRelativePath() {
        ...
        if ($this->getImage()) {
            ...
            if (file_exists($basepath . $this->getImage())) {
                $result = '/uploads/aboutus/' . $this->getId() . '/image/' . $this->getImage();
            } else {
    ...
    private function hookBeforeDelete() {
        $path = sfConfig::get('sf_upload_dir') . DIRECTORY_SEPARATOR . 'aboutus' . DIRECTORY_SEPARATOR . $this->getId() . DIRECTORY_SEPARATOR;
    ...
    public function getNextItem() {
        $c = new Criteria();
        $c->add(AboutusPeer::ID, $this->getId(), Criteria::LESS_THAN);
        $c->addDescendingOrderByColumn(AboutusPeer::ID);
        $c->setLimit(1);
        return AboutusPeer::doSelectOne($c);
    }
