...
abstract class BaseCity extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the province_id field.
	 * @var        int
	 */
	...
	/**
	 * Get the [province_id] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Set the value of [province_id] column.
	 * 
	 * @param      int $v new value
	 * @return     City The current object (for fluent API support)
	 */
	public function setProvinceId($v)
	{
		...
		if ($this->province_id !== $v) {
			$this->province_id = $v;
			$this->modifiedColumns[] = CityPeer::PROVINCE_ID;
		}

		if ($this->aProvince !== null && $this->aProvince->getId() !== $v) {
			$this->aProvince = null;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->province_id = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			...
			return $startcol + 3; // 3 = CityPeer::NUM_COLUMNS - CityPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function ensureConsistency()
	{
		...
		if ($this->aProvince !== null && $this->province_id !== $this->aProvince->getId()) {
			$this->aProvince = null;
		}
	} // ensureConsistency
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(CityPeer::ID) ) {
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aProvince !== null) {
				if (!$this->aProvince->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aProvince->getValidationFailures());
				}
			}
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $includeForeignObjects = false)
	{
		...
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getProvinceId(),
			$keys[2] => $this->getName(),
		);
		if ($includeForeignObjects) {
			if (null !== $this->aProvince) {
				$result['Province'] = $this->aProvince->toArray($keyType, $includeLazyLoadColumns, true);
			}
		}
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setProvinceId($arr[$keys[1]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CityPeer::PROVINCE_ID)) $criteria->add(CityPeer::PROVINCE_ID, $this->province_id);
	...
	{
		$copyObj->setProvinceId($this->province_id);
	...
	{
		if ($v === null) {
			$this->setProvinceId(NULL);
		} else {
			$this->setProvinceId($v->getId());
		}
	 ...
	 * @throws     PropelException
	 */
	public function getProvince(PropelPDO $con = null)
	{
		if ($this->aProvince === null && ($this->province_id !== null)) {
			$this->aProvince = ProvinceQuery::create()->findPk($this->province_id, $con);
			/* The following can be used additionally to
				 guarantee the related object contains a reference
				 to this object.  This level of coupling may, however, be
				 undesirable since it could result in an only partially populated collection
				 in the referenced object.
				 $this->aProvince->addCitys($this);
			 */
		}
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseCity:' . $name))
		{
