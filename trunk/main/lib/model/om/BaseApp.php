...
abstract class BaseApp extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the sf_app field.
	 * @var        string
	 */
	protected $sf_app;
	...
	/**
	 * Get the [sf_app] column value.
	 * 
	 * @return     string
	 */
	public function getSfApp()
	{
		return $this->sf_app;
	}
	...
	/**
	 * Set the value of [sf_app] column.
	 * 
	 * @param      string $v new value
	 * @return     App The current object (for fluent API support)
	 */
	public function setSfApp($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}
		...
		if ($this->sf_app !== $v) {
			$this->sf_app = $v;
			$this->modifiedColumns[] = AppPeer::SF_APP;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->sf_app = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			...
			return $startcol + 3; // 3 = AppPeer::NUM_COLUMNS - AppPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(AppPeer::ID) ) {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setSfApp($arr[$keys[1]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AppPeer::SF_APP)) $criteria->add(AppPeer::SF_APP, $this->sf_app);
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseApp:' . $name))
		{
