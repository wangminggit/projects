...
abstract class BaseNews extends BaseObject  implements Persistent
{
	...
	public function setCreatedByAdminUserId($v)
	{
		...
		if ($this->created_by_admin_user_id !== $v) {
			...
			$this->modifiedColumns[] = NewsPeer::CREATED_BY_ADMIN_USER_ID;
		}
	...
	public function setIsEnable($v)
	{
		...
		if ($this->is_enable !== $v || $this->isNew()) {
			$this->is_enable = $v;
			$this->modifiedColumns[] = NewsPeer::IS_ENABLE;
		}
	...
	public function setPosition($v)
	{
		...
		if ($this->position !== $v) {
			$this->position = $v;
			$this->modifiedColumns[] = NewsPeer::POSITION;
		}
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(NewsPeer::ID) ) {
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseNews:' . $name))
		{
