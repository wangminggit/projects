...
abstract class BaseBlog extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the pageview field.
	 * @var        int
	 */
	protected $pageview;
	...
	public function setSummary($v)
	{
		...
		if ($this->summary !== $v) {
			$this->summary = $v;
			$this->modifiedColumns[] = BlogPeer::SUMMARY;
		}
	...
	/**
	 * Set the value of [pageview] column.
	 * 
	 * @param      int $v new value
	 * @return     Blog The current object (for fluent API support)
	 */
	public function setPageview($v)
	{
		...
		if ($this->pageview !== $v) {
			$this->pageview = $v;
			$this->modifiedColumns[] = BlogPeer::PAGEVIEW;
		}
	...
	public function setIsEnable($v)
	{
		...
		if ($this->is_enable !== $v || $this->isNew()) {
			$this->is_enable = $v;
			$this->modifiedColumns[] = BlogPeer::IS_ENABLE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->pageview = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->is_enable = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->created_at = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
			$this->updated_at = ($row[$startcol + 7] !== null) ? (int) $row[$startcol + 7] : null;
			$this->resetModified();
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[3] => $this->getBody(),
			$keys[4] => $this->getPageview(),
			$keys[5] => $this->getIsEnable(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setPageview($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setIsEnable($arr[$keys[5]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(BlogPeer::SUMMARY)) $criteria->add(BlogPeer::SUMMARY, $this->summary);
		if ($this->isColumnModified(BlogPeer::BODY)) $criteria->add(BlogPeer::BODY, $this->body);
		if ($this->isColumnModified(BlogPeer::PAGEVIEW)) $criteria->add(BlogPeer::PAGEVIEW, $this->pageview);
		if ($this->isColumnModified(BlogPeer::IS_ENABLE)) $criteria->add(BlogPeer::IS_ENABLE, $this->is_enable);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setPageview($this->pageview);
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseBlog:' . $name))
		{
