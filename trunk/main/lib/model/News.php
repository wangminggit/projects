...
class News extends BaseNews {
    ...
    public function getImagePath() {
        $filename = "/uploads/news/" . $this->getId() . "/image/" . $this->getImage();
        return $filename;
    }

    public function getImageBasePath() {
        return sfConfig::get('sf_upload_dir') . DIRECTORY_SEPARATOR . 'news' . DIRECTORY_SEPARATOR . $this->getId() . DIRECTORY_SEPARATOR . 'image' . DIRECTORY_SEPARATOR;
    }
    ...
    public function getImageRelativePath() {
        ...
        if ($this->getImage()) {
            ...
            if (file_exists($basepath . $this->getImage())) {
                $result = '/uploads/news/' . $this->getId() . '/image/' . $this->getImage();
            } else {
    ...
    private function hookBeforeDelete() {
        $path = sfConfig::get('sf_upload_dir') . DIRECTORY_SEPARATOR . 'news' . DIRECTORY_SEPARATOR . $this->getId() . DIRECTORY_SEPARATOR;
    ...
    public function getNextItem() {
        $c = new Criteria();
        $c->add(NewsPeer::ID, $this->getId(), Criteria::LESS_THAN);
