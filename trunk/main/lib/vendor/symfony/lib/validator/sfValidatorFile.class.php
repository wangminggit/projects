...
class sfValidatorFile extends sfValidatorBase
{
  ...
  protected function configure($options = array(), $messages = array())
  {
    ...
    $this->addMessage('max_size', '文件太大了 (最大 %max_size% 字节).');
    $this->addMessage('mime_types', '无效的文件类型 (%mime_type%).');
    $this->addMessage('partial', '上传文件只有部分被上传.');
    $this->addMessage('no_tmp_dir', '缺少一个临时文件夹.');
    $this->addMessage('cant_write', '未能将文件写入磁盘.');
    $this->addMessage('extension', '文件上传被扩展停止.');
  }
